...
const PIN = { left: "34%", top: "62%" } as const;
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
      <div className="p-5 pb-0">
        <h2 className="text-lg font-semibold text-foreground">Find Sea Saba</h2>
      </div>

      {/* Photo with pin — entire image is clickable */}
      <div
        role="button"
        tabIndex={0}
        aria-label="Open Sea Saba Dive Center in Google Maps, opens in a new tab"
        onClick={openMaps}
        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") openMaps(); }}
        className="relative mt-4 block aspect-[16/9] w-full cursor-pointer overflow-hidden"
      >
        <Image
          src="/images/optimized/fort-bay-harbor-saba.webp"
          alt="Fort Bay Harbor, Saba — home of Sea Saba Dive Center"
          fill
          loading="eager"
          priority
        ...
        >
          ...
          >
            <a
              ...
              className="flex min-w-[10rem] flex-col items-start rounded-xl bg-white px-4 py-3 shadow-xl"
            >
          ...
          {/* Pin marker — stops propagation so tap toggles tooltip without opening Maps */}
          <button
            type="button"
            aria-label="Sea Saba Dive Center location — tap to see details"
          ...
          >
            ...
            />
            <span
              className="relative flex h-8 w-8 items-center justify-center rounded-full transition-transform duration-200 group-hover:scale-110"
              style={{
