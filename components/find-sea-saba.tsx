...
const HERO_IMAGE_SRC = "/images/optimized/fort-bay-harbor-saba.webp";
...
export function FindSeaSaba({ locale = DEFAULT_LOCALE }: { locale?: Locale }) {
  const ui = uiFor(locale).findSeaSaba;
  return (
        ...
        >
          ...
          >
            <a
              ...
              aria-label={ui.getDirectionsAria}
              className="flex min-w-[10rem] flex-col items-start rounded-xl bg-white px-4 py-3 shadow-xl no-underline"
            >
              <span className="text-sm font-semibold text-foreground">{ui.tooltipName}</span>
              <span className="mt-0.5 text-xs text-muted-foreground">{ui.tooltipPlace}</span>
