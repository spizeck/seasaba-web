...
| Contact | The form builds a structured inquiry and opens it in the visitor's own email app via `mailto:` (`info@seasaba.com` recipient, canonical subject, structured body); WhatsApp remains a handoff alternative. Nothing is transmitted or stored by the website — server-side email sending was removed (see issue #104) |
...
| --- | --- | --- |
| Unit | Vitest, `tests/unit` | Measurement/date conversion; reference normalization and legacy guide formats; grouping identities, maximum sighting counts and input immutability; analytics URL sanitization/domain classification; metadata, sitemap and redirect contracts; contact email subject/body construction and `mailto:` URI encoding; Respond.io widget URL construction and `chat:sent`→conversation-start deduping |
| Integration | Vitest + React Testing Library, `tests/integration` | Real component interactions; required fields, invalid-email recovery, course prefill, contact email handoff content/truthful status, WhatsApp handoff; Checkfront inventory mapping/success/script error/timeout/render failure; Firestore join, empty data, permission-denied and service failures; dive filters, ordering, pagination, selection and export; real jsPDF generation with download boundary replaced; Respond.io widget script injection (env gating, load-event deferral, remount dedup, graceful failure, analytics wiring against a stubbed `$respond`) |
...
- The Respond.io Website Chat widget is disabled in the test build (`NEXT_PUBLIC_RESPOND_IO_CID` is empty in `scripts/build-test.mjs`), so browser tests never contact Respond.io. The loader is exercised in jsdom against a stubbed `window.$respond`; real widget rendering and Coral/workflow routing are verified manually on a deployed environment, like the `mailto:` handoff.
...
4. Verify configured Cookiebot/GTM behavior in a consent-enabled staging deployment and test the WhatsApp app handoff on physical iOS/Android devices. External consent services and OS apps are isolated from CI. The contact form's mailto: handoff needs one manual post-deploy verification on a device with a configured mail client: submit an inquiry, confirm the draft opens addressed to `info@seasaba.com` with the canonical subject/body, send it, and confirm Respond.io files it under the sender's own contact. The Website Chat widget likewise needs one manual post-deploy verification per visitor identity: open the launcher, submit the pre-chat form (first name + email), send a message, and confirm a distinct Respond.io contact/conversation appears, Coral/the assignment workflow runs, staff can reply, and a second test visitor does not collapse into the first contact.
