...
| `lib/respond-io.ts` + `components/respond-io-widget.tsx` | Website Chat channel — the native Respond.io launcher (channel `451154`, public widget cId via `NEXT_PUBLIC_RESPOND_IO_CID`) injected after window load. Widget appearance, pre-chat fields, popup message and workflow routing are Respond.io dashboard settings, not website code. Per-visitor identity is handled inside the widget's own iframe |
