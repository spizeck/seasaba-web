...
| `data/donations.ts` | `DONATION_RECIPIENTS` — owner-approved organizations/causes shown on `/donate` (empty until recipients are approved; the page renders an in-progress state). Entries link out to each organization's own site — Sea Saba never processes donations |
